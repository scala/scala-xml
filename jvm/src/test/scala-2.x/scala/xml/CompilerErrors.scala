...
class CompilerErrors extends CompilerTesting {
  ...
  def t7185() = {
    ...
    val thing =
      if (
        versionNumberString.startsWith("2.11") || versionNumberString
          .startsWith("2.12")
      ) "method value"
      else "method"
    expectXmlError(
      s"""|overloaded $thing apply with alternatives:
...
class CompilerTesting {
  ...
  def errorMessages(errorSnippet: String, compileOptions: String = "")(
      code: String
  ): List[String] = {
  ...
  def expectXmlErrors(msgCount: Int, msg: String, code: String) = {
    ...
    assert(
      errorCount == msgCount,
      s"$errorCount occurrences of \'$msg\' found -- expected $msgCount in:\n${errors mkString "\n"}"
    )
  }
}
