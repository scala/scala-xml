...
object ReuseNodesTest {
  ...
  class OriginalTranformr(rules: RewriteRule*)
      extends RuleTransformer(rules: _*) {
    override def transform(ns: Seq[Node]): Seq[Node] = {
      ...
      if (xs2.isEmpty) ns
        ...
        (xs1 map (_._2)) ++ xs2.head._1 ++ transform(ns drop (xs1.length + 1))
    }
    override def transform(n: Node): Seq[Node] = super.transform(n)
  }

  class ModifiedTranformr(rules: RewriteRule*)
      extends RuleTransformer(rules: _*) {
    override def transform(ns: Seq[Node]): Seq[Node] = {
  ...
  class AlternateTranformr(rules: RewriteRule*)
      extends RuleTransformer(rules: _*) {
    override def transform(ns: Seq[Node]): Seq[Node] = {
      val xs = ns.toStream map transform
      val (xs1, xs2) = xs zip ns span { case (x, n) => unchanged(n, x) }
...
class ReuseNodesTest {
  ...
  def transformReferentialEqualityOnly(rt: RuleTransformer) = {
    val original =
      <changed><change><lost/><a><b><c/></b></a></change><a><b><c/></b></a></changed>
  ...
  def recursiveAssert(original: Seq[Node], transformed: Seq[Node]): Unit = {
    original zip transformed foreach { case (x, y) =>
      recursiveAssert(x, y)
    }
  }
