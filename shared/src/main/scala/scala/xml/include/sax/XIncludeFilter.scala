...
class XIncludeFilter extends XMLFilterImpl {
  ...
  private def includeTextDocument(url: String, encoding1: String): Unit = {
    ...
    try {
      ...
      case e: MalformedURLException =>
        val ex = new UnavailableResourceException(
          "Unresolvable URL " + url
            + getLocation()
        )
        ex.setRootCause(e)
      ...
      if (encodingFromHeader != null)
        ...
        if (contentType != null) {
          ...
          if (
            ...
            || (contentType
              .startsWith("application/") && contentType.endsWith("+xml"))
          ) {
      ...
      case e: UnsupportedEncodingException =>
        throw new SAXException(
          "Unsupported encoding: "
            + encoding + getLocation(),
          e
        )
      case e: IOException =>
        throw new SAXException(
          "Document not found: "
            + source.toExternalForm() + getLocation(),
          e
        )
    }
  ...
  private def includeXMLDocument(url: String): Unit = {
    val source =
      try new URL(bases.peek(), url)
        ...
        case e: MalformedURLException =>
          val ex = new UnavailableResourceException(
            "Unresolvable URL " + url + getLocation()
          )
          ex setRootCause e
          throw new SAXException("Unresolvable URL " + url + getLocation(), ex)
      }
    ...
    try {
        ...
        try XMLReaderFactory.createXMLReader()
          ...
          case e: SAXException =>
            try XMLReaderFactory.createXMLReader(XercesClassName)
              ...
              case _: SAXException =>
                return System.err.println("Could not find an XML parser")
            }
        }
      ...
      case e: IOException =>
        throw new SAXException(
          "Document not found: " + source.toExternalForm() + getLocation(),
          e
        )
    }
  }
