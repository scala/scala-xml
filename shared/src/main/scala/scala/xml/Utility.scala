...
object Utility extends AnyRef with parsing.TokenTests {
  ...
  def sort(md: MetaData): MetaData = if ((md eq Null) || (md.next eq Null)) md
  else {
    val key = md.key
  ...
  def collectNamespaces(nodes: Seq[Node]): mutable.Set[String] =
    nodes.foldLeft(new mutable.HashSet[String]) { (set, x) =>
      collectNamespaces(x, set); set
    }
  ...
  def toXML(
    ...
    serialize(
      ...
      decodeEntities,
      preserveWhitespace,
      if (minimizeTags) MinimizeMode.Always else MinimizeMode.Never
    )
  }
    ...
    x match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "Don't know how to serialize a " + x.getClass.getName
        )
    }
  }
  ...
  def hashCode(
    ...
    scala.util.hashing.MurmurHash3
      .orderedHash(label +: attribHashCode +: scpeHash +: children, pre.##)
